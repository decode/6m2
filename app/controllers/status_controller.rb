class StatusController < ApplicationController
  ...
  def total
    ...
    @month_tasks = Task.where(:created_at => Time.now.at_beginning_of_month..Time.now.at_end_of_month).order('created_at ASC')
    @month_tasks_sum = @month_tasks.sum('price')
    @month = [0]*31
    @month_tasks.each do |t|
      @month[t.created_at.day-1] = @month[t.created_at.day-1] + 1
    end
